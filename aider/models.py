...
class ModelInfoManager:
    ...
    def _load_bundled_metadata(self):
        """Load bundled model metadata from aider.resources.model-metadata.json"""
        try:
            bundled_data = json5.loads(
                importlib.resources.read_text("aider.resources", "model-metadata.json")
            )
            if bundled_data:
