...
class Commands:
    ...
    def cmd_tokens(self, args):
        ...
        if not limit:
            self.io.tool_output(f"{cost_pad}{fmt(0)} tokens remaining (context window unknown)")
    ...
    def cmd_add(self, args):
        ...
        for matched_file in sorted(all_matched_files):
            ...
            if abs_file_path in self.coder.abs_fnames:
                ...
                if is_image_file(matched_file) and not (self.coder.main_model.info.get("supports_vision") or "vision" in self.coder.main_model.name):
                    self.io.tool_error(
