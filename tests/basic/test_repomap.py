...
class TestRepoMapAllLanguages(unittest.TestCase):
    ...
    def test_get_tags_handles_exception(self):
        ...
        with patch.object(RepoMap, "get_tags_raw", side_effect=ValueError("boom")):
            io = InputOutput()
            repo_map = RepoMap(main_model=Model("gpt-3.5-turbo"), root="/tmp", io=io)
            result = repo_map.get_tags("/nonexistent/file.cs", "file.cs")
